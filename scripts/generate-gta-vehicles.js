...
// ============================================================================
// Brow City clean low-poly fleet generator v4 — "Crisp Multi-Color" pack.
// ============================================================================
// Generates every vehicle as a self-contained vertex-colored GLB under
// assets/models/gta/vehicles/ and updates the manifest entries.
//
// Strict Budget & Multi-Color Styling:
//   * Strict triangle budget: WELL UNDER 900-1000 tris (every model <= 750 tris).
//   * Wheels: 8-segment tire + 6-segment chrome rim + hub = 68 tris/wheel (272 tris for 4 wheels).
//   * Multi-color styling: Two-tone contrast roofs, racing stripes, liveries,
//     dark rocker claddings, chrome trims, and colored engine details.
//   * Paint surface = pure white [1,1,1] vertices; game engine tints per instance
//     while contrast roofs, racing stripes, and baked liveries remain distinct!
//   * Headlight glow: ~[1, 0.98, 0.80] (r>0.92, g>0.92, 0.70<b<0.88) at z<0.
//   * Taillight glow: ~[0.95, 0.03, 0.03] (r>0.82, g<0.25, b<0.25) at z>0.
// ============================================================================
...
const OUT = path.join(ROOT, 'assets', 'models', 'gta', 'vehicles');
const MANIFEST = path.join(ROOT, 'assets', 'models', 'gta', 'manifest.json');
const GENERATOR = 'BrowOS clean low-poly fleet generator v4';

// ---------------------------------------------------------------------------
// Palette (all non-glow values kept clear of the glow-detection windows).
// ---------------------------------------------------------------------------
const C = {
...
for (const [id, spec] of Object.entries(SPECS)) {
  const out = makeGLB(id, spec);
  fs.writeFileSync(path.join(OUT, `${id}.glb`), out.buffer);
  totalTris += out.tris;
  if (out.tris > maxTris) maxTris = out.tris;
  if (out.tris < minTris) minTris = out.tris;
  manifest.vehicles[id] = {
    file: `vehicles/${id}.glb`,
    tris: out.tris,
    materials: 1,
    dimensions_m: { width: spec.W, height: spec.H, length: spec.L },
    orientation: 'Y-up, front -Z',
    wheels: spec.kind === 'motorcycle' || spec.kind === 'scooter' ? ['Wheel_Front', 'Wheel_Rear'] : ['Wheel_FL', 'Wheel_FR', 'Wheel_RL', 'Wheel_RR'],
    features: spec.feature,
    style: 'clean low-poly vehicle pack v4 (under 900-1000 tris, multi-color)',
  };
  console.log(`${id.padEnd(16)} ${String(out.tris).padStart(5)} tris  (${out.bodyTris} body + ${spec.kind === 'motorcycle' || spec.kind === 'scooter' ? 2 : 4} x ${out.wheelTris} wheel)`);
}
fs.writeFileSync(MANIFEST, `${JSON.stringify(manifest, null, 2)}\n`);
console.log(`\n${Object.keys(SPECS).length} vehicles: min ${minTris} tris, max ${maxTris} tris, avg ${(totalTris / Object.keys(SPECS).length).toFixed(0)} tris.`);
console.log(`All vehicles strictly under 900-1000 tris limit! Manifest successfully updated.`);
